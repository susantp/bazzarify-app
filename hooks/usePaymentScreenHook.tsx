...
export default function usePaymentScreenHook(id?: string) {
  ...
  const componentMap: Record<PaymentMethodType["id"], React.ReactNode> = {
    card: (
      <>
        <CardPaymentComponent btnLabel={"Pay"} />
        <BottomActionView onLayoutEvent={handleBottomViewLayoutEvent}>
          <CardPaymentBottomActionView totalPrice={399} subTotalPrice={399} />
        </BottomActionView>
      </>
    ),
    cod: (
      <>
        <CODPaymentComponent btnLabel={"Pay"} />
        <BottomActionView onLayoutEvent={handleBottomViewLayoutEvent}>
  ...
  const paymentMethodSections: PaymentMethodSection[] = useMemo(
    () => [
      {
  ...
  const paymentMethodById: PaymentMethodType | undefined = useMemo(
    () => _.find(_.flatten(_.pluck(paymentMethodSections, "methods")), { id }),
    [id, paymentMethodSections],
  );
