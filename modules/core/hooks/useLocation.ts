...
export function useLocation() {
  const setLat = useSetRecoilState(latitudeAtom);
  const setLng = useSetRecoilState(longitudeAtom);
  const setGeocode = useSetRecoilState(geocodeAddressAtom);
  const setError = useSetRecoilState(locationErrorAtom);
